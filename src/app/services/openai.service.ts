...
export class OpenaiService {
  ...
  generateTestsFromAC(criteria: string) {
    return from(
        ...
        messages: [
          {
            ...
            content: 
              ...
              "Scenario" and "Expected Result" field that are both strings.`
          },
          {
            role: "user",
            content: `Given the following Acceptance Criteria (in brackets) generate all tests from the criteria and
                      up to 10 additional edge case scenarios that may be missed by the acceptance criteria instructions.
                      [${criteria}]`
          },
        ],
      })
